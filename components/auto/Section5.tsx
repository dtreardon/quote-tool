...
export function Section5() {
  ...
  return (
    <SectionCard number={5} title="Coverage">
      {/* BI/PD + UM/UIM */}
      <div className="mb-4 bg-[#f7f4ee] border border-[#d0cdc8] rounded p-3.5">
        <div className="text-[11px] font-bold text-[#666] uppercase tracking-[0.05em] mb-2">Liability & UM/UIM</div>
        <div className="grid grid-cols-2 gap-3.5">
          <Field label="Bodily Injury (BI)">
          ...
          <Field label="Property Damage (PD)">
          ...
          <Field label="UM — Uninsured Motorist: Bodily Injury (BI)">
            <select value={form.cov_um_bi} onChange={e => update({ cov_um_bi: e.target.value })} className={selectCls()}>
              {BI_OPTIONS.map(o => <option key={o} value={o}>{o || 'Select limits…'}</option>)}
            </select>
          </Field>
          <Field label="UM — Uninsured Motorist: Property Damage (PD)">
