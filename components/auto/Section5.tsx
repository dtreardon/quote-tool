  ...
  return (
    <SectionCard number={5} title="Coverage">
      ...
      <div className="flex gap-3.5 flex-wrap mb-4 bg-[#f7f4ee] border border-[#d0cdc8] rounded p-3.5">
        ...
        <Field label="UM — Uninsured Motorist: Bodily Injury (BI)" className="flex-1 min-w-44">
          <select value={form.cov_um_bi} onChange={e => update({ cov_um_bi: e.target.value })} className={selectCls()}>
            {BI_OPTIONS.map(o => <option key={o} value={o}>{o || 'Select limits…'}</option>)}
          </select>
        </Field>
        <Field label="UM — Uninsured Motorist: Property Damage (PD)" className="flex-1 min-w-44">
          <select value={form.cov_um_pd} onChange={e => update({ cov_um_pd: e.target.value })} className={selectCls()}>
            {PD_OPTIONS.map(o => <option key={o} value={o}>{o || 'Select limit…'}</option>)}
          </select>
        </Field>
        <Field label="UIM — Underinsured Motorist: Bodily Injury (BI)" className="flex-1 min-w-44">
          <select value={form.cov_uim_bi} onChange={e => update({ cov_uim_bi: e.target.value })} className={selectCls()}>
            {BI_OPTIONS.map(o => <option key={o} value={o}>{o || 'Select limits…'}</option>)}
          </select>
        </Field>
        <Field label="UIM — Underinsured Motorist: Property Damage (PD)" className="flex-1 min-w-44">
