...
function evaluateCarriers(input: any) {
  ...
  const limitedCarriers = input.top3Only ? eligibleCarriers.slice(0, 3) : eligibleCarriers;
  const hasBrokerCarrier = limitedCarriers.some((carrier: any) => carrier.key === "brokers");
  const withBrokerFallback =
    limitedCarriers.length < 3 && !hasBrokerCarrier
      ? [...limitedCarriers, { label: "Brokers", reason: "Submit for quotes", isBrokerFallback: true }]
      : limitedCarriers;
...
export default function CarrierGuidePage() {
  ...
  useEffect(() => {
    if (!GOOGLE_MAPS_API_KEY) { setStatus("missing-key"); return; }
  ...
  useEffect(() => {
    ...
    autocomplete.addListener("place_changed", () => {
      ...
      if (lat == null || lng == null) { setError("Could not get location from selected address."); return; }
      ...
      const filteredFeatures = (coastline as any).features.filter((feature: any) => {
        ...
        return coords.some((c: number[]) => {
          ...
          return featureLng >= -87 && featureLng <= -75 && featureLat >= 24 && featureLat <= 37;
        });
      });
      ...
      setForm(prev => ({ ...prev, address: place.formatted_address || "", zip, city, state, county, distanceToCoast: Number(distance.toFixed(2)), lat, lng }));
      setError("");
      setResults(null);
    });
  ...
  const staticMapUrl = useMemo(() => {
    if (!GOOGLE_MAPS_API_KEY || form.lat == null || form.lng == null) return "";
  ...
  const zillowUrl = useMemo(() => {
    ...
    return `https://www.zillow.com/homes/${form.address.replace(/,/g, "").replace(/\s+/g, "-")}_rb/`;
  }, [form.address]);

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!form.address.trim()) { setError("Enter an address."); return; }
    if (!form.zip || form.zip.length < 5) { setError("Select a valid address from the dropdown."); return; }
    if (!form.buildYear) { setError("Enter a build year."); return; }
    if (!form.roofYear) setForm(prev => ({ ...prev, roofYear: prev.buildYear }));
    const evaluated = evaluateCarriers({
      zip: form.zip,
      state: form.state,
      county: form.county,
      buildYear: Number(form.buildYear),
      roofYear: Number(form.roofYear || form.buildYear),
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="w-full bg-white border-b">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-center">
          <div className="relative h-14 w-64">
            <Image src="/logo.png" alt="Robinson & Associates logo" fill className="object-contain" priority />
          </div>
        </div>
      </div>
              ...
              <form onSubmit={handleSubmit} className="flex h-full flex-col space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
                <div>
                  <input
                    ...
                    value={form.address}
                    onChange={e => setForm(prev => ({ ...prev, address: e.target.value, zip: "", city: "", state: "", buildYear: "", roofYear: "", distanceToCoast: null, lat: null, lng: null }))}
                  />
                  {form.address && zillowUrl && (
                    <a href={zillowUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-blue-600 hover:underline mt-2">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <input className="w-full rounded-lg border border-gray-200 bg-gray-50 p-3" placeholder="ZIP" value={form.zip} readOnly />
                  <input className="w-full rounded-lg border border-gray-200 bg-gray-50 p-3" placeholder="City" value={form.city} readOnly />
                  <input className="w-full rounded-lg border border-gray-200 bg-gray-50 p-3" placeholder="State" value={form.state} readOnly />
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <select className="w-full rounded-lg border border-gray-300 p-3 bg-white" value={form.policyType} onChange={e => setForm({ ...form, policyType: e.target.value })}>
                    <option value="HO">HO</option>
                    <option value="DP">DP</option>
                  </select>
                  <select className="w-full rounded-lg border border-gray-300 p-3 bg-white" value={form.roofType} onChange={e => setForm({ ...form, roofType: e.target.value })}>
                ...
                {roofAge !== null && !Number.isNaN(roofAge) && (
                  <div className="text-xs text-gray-500">Estimated roof age: {roofAge} years</div>
                )}
                <div className="grid gap-2 sm:grid-cols-3">
                  {[
                    { key: 'hasSolar', label: 'Solar' },
                    { key: 'mobileHome', label: 'Mobile Home' },
                    { key: 'top3Only', label: 'Top 3 Only' },
                  ].map(({ key, label }) => (
                    <label key={key} className="flex items-center gap-2 rounded-lg border border-gray-200 px-3 py-2 text-sm">
                ...
                <button className="rounded-lg bg-blue-600 px-4 py-3 font-medium text-white transition hover:bg-blue-700 self-start">Find Carriers</button>
              </form>
            </div>
            <div className="h-full">
              <div className="flex h-full flex-col overflow-hidden rounded-2xl border bg-white shadow-sm">
                ...
                {staticMapUrl ? (
                  <div className="aspect-[4/3] bg-gray-100">
                    <img src={staticMapUrl} alt="Satellite view of selected property" className="h-full w-full object-cover" />
                  </div>
                ) : (
          ...
          <div className="rounded-2xl border bg-white p-5 shadow-sm">
            ...
            {!results ? (
              ...
              <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-3">
                {results.map((r, i) => (
                  <div key={i} className={`rounded-xl border p-4 ${r.isBrokerFallback ? "border-gray-300 bg-gray-50" : "border-gray-200 bg-white"}`}>
                    <div className="flex items-start justify-between gap-3">
                            ...
                            {r.alerts?.length > 0 && (
                              <div className="mt-2 space-y-1">
                                {r.alerts.map((alert: string, idx: number) => (
                                  <div key={idx} className="flex items-start gap-2 rounded-md border border-yellow-200 bg-yellow-50 px-2 py-1 text-xs text-yellow-800">
                      ...
                      {!r.isBrokerFallback && (
                        <div className="rounded-full bg-gray-100 px-3 py-1 text-sm font-medium text-gray-800">Score: {r.score}</div>
                      )}
                    </div>
                  </div>
                ))}
      ...
      <div className="border-t bg-white">
        <div className="max-w-7xl mx-auto px-6 py-4 text-center text-xs text-gray-500">© 2026 Reardon Insurance, LLC</div>
      </div>
    </div>
  );
}
